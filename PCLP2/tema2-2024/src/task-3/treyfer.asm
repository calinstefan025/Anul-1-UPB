...
treyfer_crypt:
	...
	mov ecx , [num_rounds] ; folosim la loop runde
...
loop_criptare:
	mov byte al , [esi + ebx] ; byte text[i]
	mov byte dl , [edi + ebx] ; byte key[i]
	add al , dl ; t = t + key[i]

	movzx eax , al ; t
	mov dl , byte [sbox + eax] ; sbox[t]
	mov al , dl ; t = sbox[t]
	...
	add ebx , 1 ; i = i + 1
	add al , byte [esi + ebx] ; t = t + text[i + 1]
	sub ebx , 1 ; revenim la i-ul curent

	rol al , 1 ; rotim la stanga cu 1 bit

	add ebx , 1 ; i = i + 1
	mov byte [esi + ebx] , al ; text[i] = t

	jmp loop_criptare

ultimul_byte:
	add al , byte [esi] ; t = t + text[0]
	rol al , 1 ; rotim la stanga cu 1 bit
	mov byte [esi] , al ; text[0] = t
	loop loop_runde
